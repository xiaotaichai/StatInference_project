...
plot(fit_oil2)
```
It seems the polynomial model fits the data better than the linear model since the adjusted r-squared is higher than the linear model's. Also, all coefficients are siginificant. From the QQ plot, the residuals have a positively skewed distribution, so we could also try log regression model.

```{r}
fit_oil3 <- lm(log(Net) ~ `Price.Close`, data = oil_combo)
summary(fit_oil3)
...
Compare the linear regression model, polynomial model, and log regression model, we found that the log regression model has a highest adjusted R-squared value and has a lowest p-value.
